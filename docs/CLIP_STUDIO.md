...
**Call idle (looping FaceTime hold):** the `idle` intent is what you see after Accept and after every reaction. Attach a short MP4 on an idle slot, then **Use as call idle**. The demo loops that clip with a short crossfade back from reactions. If nothing is attached yet, the call holds the correct dog still — not the colored placeholder slab. Preference is stored on the dog in localStorage and does not wipe other attachments.
